...
def goal_overlap(inputs, threshold=0.75, **kwargs):  # 找出物体放置位置与“目标高度区域”的重叠度足够高的位置。
  ...
  #把“当前高度低于目标高度”的位置提取出来，并用 0/1 表示。
  b = (inputs[0][:,:,0] < inputs[0][:,:,1]).astype('int')  # 场景中还需要填充的目标位置（目标高度比当前高度高），将布尔数组转换为整数数组
  n = (inputs[1][:,:,0] > 0).astype('int') # 待放置物体占据的格子
  f = signal.correlate2d(b, n, mode='valid')
   # 使用二维互相关，把物体高度图 n 当作模板在 b 上滑动匹配
   # 每个位置 f[i,j] 表示物体放在 (i,j) 时与目标高度重叠的格子数
  return f >= threshold*f.max() #只保留重叠度达到阈值的放置位置，如果f大于f.max()的75%，就返回true，否则返回false


# 把字符串名称映射到对应的函数，方便在程序中 动态选择策略，而不必写很多 if/else，也方便用 配置文件（如 Gin-config） 来指定策略
methods = {
  'random': random,
  'correlate':correlate,
  'height': height,
...
# 启发式策略就是在没有完整学习或优化的情况下，依靠经验法则、规则或简单计算来指导决策的策略。
#特点：   无需学习：不依赖大量训练数据或强化学习算法
#        规则驱动：用“经验规则”或简单公式来评估动作优劣
#        快速：计算开销低，通常用于基线（baseline）或初始化策略
#        可解释：你可以看懂为什么选择这个动作
#生成一些候选动作（可以是随机的）
...
class Baseline(agents.PyGreedy): #Baseline 继承自 agents.PyGreedy，说明是一个贪心策略代理（Greedy Agent）
  def __init__(。# 每次你创建一个新对象，__init__ 都会执行一次，初始化这个对象的属性。
    self, 
    method='random',  # 选择使用哪种启发式策略，默认 'random'
    goal=True,        # 是否考虑“目标位置重叠”
    minorder=1,       # 局部极小值范围（用于局部最优选择）
    value=False,      # 是否返回评分值
    unravel=False,    # 是否将二维索引展开为一维
    batched=False,    # 是否支持批量计算
    batchwise=False,  
    **kwargs,         # 额外参数，传给具体策略函数
